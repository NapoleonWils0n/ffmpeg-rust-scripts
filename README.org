...
Your note is correct for a standard cargo build. However, since you are using Nix, you have a second "pure" option:
...
Result: This will place your binaries in ./result/bin/ instead of target/release/.

Advantage: This ensures the build is 100% reproducible and isolated from your local system state.

Instead of a simple mv (which might fail if the source is the read-only Nix store), you should copy them. When you copy a binary out of the Nix store, it keeps its "rpath" (it still knows exactly where to find its library dependencies in /nix/store), so it will continue to work perfectly.

Run this command to copy all 32 binaries at once:
...
Binaries go to: target/x86_64-unknown-linux-musl/release/
...
One of the main reasons to use musl is to ensure the binary has no external dependencies. You can verify this by running the ldd command on one of the new binaries:

#+begin_src sh
ldd target/x86_64-unknown-linux-musl/release/scene-detect-auto
#+end_src

Expected Output: It should say statically linked or not a dynamic executable. This confirms that a user on Ubuntu, Debian, or Arch can just download that file and run it immediately (provided they have ffmpeg installed).
...
Why fd is better here:
The ! -name "*.*" logic in standard find can sometimes be finicky depending on the shell, but fd's -E (exclude) flag is very robust. This will cleanly grab your 32 binaries and ignore all the compiler junk.

Use this find command. It is the safest way to ensure you only get the executable binaries and nothing else

#+begin_src sh
find target/x86_64-unknown-linux-musl/release/ -maxdepth 1 -type f ! -name "*.*" -exec cp {} ~/Desktop/build/linux/ \;
#+end_src
...
Binaries go to: target/x86_64-pc-windows-gnu/release/

#+begin_src sh
cargo build --release --target x86_64-pc-windows-gnu
#+end_src

To copy the Windows builds

#+begin_src sh
fd -t f -e exe . target/x86_64-pc-windows-gnu/release/ -x cp {} ~/Desktop/build/windows/
#+end_src

*** mac

For a Mac user, the process is very similar to Linux and arguably easier than Windows.

Setup: They just need the Xcode Command Line Tools (usually already installed if they do any dev work, or triggered by typing gcc or git in the terminal) and Rustup.

Performance: Modern Apple Silicon (M1/M2/M3) chips compile Rust incredibly fast.

Dependencies: Since you mentioned your users must provide their own ffmpeg, yt-dlp, etc., Mac users usually manage these easily via Homebrew (brew install ffmpeg yt-dlp).
...
note you need to export SDL_VIDEODRIVER=wayland in your shell config
