...
Smart Encoding: Automatically detects hardware capabilities to use HEVC NVENC for maximum speed, falling back to libx264 (CRF 18) if a GPU is not present.

Smart Audio: Detects the source audio codec; it performs a bit-perfect stream copy if the audio is already AAC, or transcodes to AAC if it is not, ensuring maximum compatibility without unnecessary quality loss.
...
Audio Preservation: Uses a bit-perfect stream copy (-c:a copy) for the audio, ensuring that only the video is re-encoded to apply the filter while the original audio quality remains untouched.
