...
Install ffmpeg with home-manager by adding the following package.
...
install yt-dlp with home-manager
...
This will allow the scripts to access the ffmpeg and ffprobe binaries from the command line.
...
You can install both yt-dlp (for downloading media) and Deno (for any secondary script requirements) in a single command.
...
**** ffmpeg yt-dlp install

Run the following command in the terminal to install ffmpeg and yt-dlp with homebrew
...
Run the following command in the terminal to install yt-dlp and deno
